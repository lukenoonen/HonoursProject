...
Ptr<WeightedGraph> ShapefileParser::createInternal() const
{
	...
	for (const auto& entry : fs::directory_iterator(_directory))
	{
		...
		{
			const int length = DBFReadIntegerAttribute(
				handle, shape, lengthIndex
			);
			const Str startNode = DBFReadStringAttribute(
				handle, shape, startNodeIndex
			);
			const Str endNode = DBFReadStringAttribute(
				handle, shape, endNodeIndex
			);
			...
			const auto   weight = static_cast<double>(length);

			while (result->numVertices() <= source) { result->addVertex({}); }
			while (result->numVertices() <= target) { result->addVertex({}); }
