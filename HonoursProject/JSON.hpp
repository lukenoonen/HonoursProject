...
#define JSON_CREATE(_type) void from_json(const JSON&, _type&);
...
#define JSON_ARG(_type, _name) auto(_name) = json_arg<_type>(json, #_name);

#define JSON_ARG_FALLBACK(_type, _name, _fallback)                   \
	auto(_name) = json_arg_fallback<_type>(json, #_name, _fallback);

#define JSON_ARG_SINGLE(_type, _name)             \
	auto(_name) = json_arg<_type>(json, nullptr);

#define JSON_ARG_SINGLE_FALLBACK(_type, _name, _fallback)    \
	auto(_name) = json_arg<_type>(json, nullptr, _fallback);

#define JSON_FABRICATE(...) data = Type{__VA_ARGS__};

#define JSON_END() }

#define JSON_CREATE_FACTORY(_type) JSON_CREATE(Ptr<_type>)
