#include "ShortcutHierarchyAnalyser.h"
...
void ShortcutHierarchyAnalyser::analyseFile( const FilePath& filepath ) const
{
	...
	{
		g_logger.log( "Failed to read shortcut hierarchy from {}\n", filepath.string() );
		return;
	}
	...
	for (const auto& layer : hierarchy._hierarchy)
	{
		...
		g_logger.log( "V = \n[\n" );
		layer.vertexMap( [&layer]( const auto v ) {
			g_logger.log( "\t{}\n", layer.toSource( v ) + 1 );
			return false;
		} );
		g_logger.log( "]\n" );
		g_logger.log( "E = \n[\n" );
		layer.edgeMap( [&layer]( const auto e ) {
			const auto u = layer.source( e );
			const auto v = layer.target( e );
			g_logger.log( "\t({}, {}) : {}\n", layer.toSource( u ) + 1, layer.toSource( v ) + 1, layer[e].weight() );
			return false;
		} );
		g_logger.log( "]\n" );
