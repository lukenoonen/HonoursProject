...
template <class V, class E, template <class, class> class F>
inline Opt<typename BaseGraph<V, E, F>::Edge> BaseGraph<V, E, F>::addEdge(
	Vertex s,
	Vertex t,
	E      data
)
...
inline void BaseGraph<V, E, F>::removeVertices(const Set<Vertex>& remove)
{
	...
	edgeMap(
		[this, &toUpdated, &updated](const auto e) {
			...
			boost::add_edge(
				uSearch->second, vSearch->second, std::move(get(e)), updated
			);
			return false;
		}
	);
...
template <class V, class E, template <class, class> class F>
inline Opt<typename BaseGraph<V, E, F>::Edge> BaseGraph<V, E, F>::edge(
	Vertex s,
	Vertex t
) const
{
...
template <class V, class E, template <class, class> class F>
inline Set<typename BaseGraph<V, E, F>::Vertex> BaseGraph<V, E, F>::
	connectedComponent(Vertex v) const
{
	Set<Vertex> result;
	dfs(v,
...
template <class V, class E, template <class, class> class F>
inline Pair<
	Set<typename BaseGraph<V, E, F>::Vertex>,
	Set<typename BaseGraph<V, E, F>::Vertex>>
BaseGraph<V, E, F>::largestConnectedComponent() const
{
	Set<Vertex> outside;
	Set<Vertex> inside;
	vertexMap(
...
inline bool BaseGraph<V, E, F>::edgeMap(P predicate) const
{
	...
	{
		const auto e = *it++;
		if (_filter(e) || _filter(source(e)) || _filter(target(e)))
		{
			continue;
		}
